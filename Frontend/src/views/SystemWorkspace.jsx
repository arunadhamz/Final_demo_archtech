...
export default function SystemWorkspace({ project: activeProject }) {
  ...
  const [activeTab, setActiveTab] = useState('syrs');
  ...
  const [codeFiles, setCodeFiles] = useState({
    'system/config/sys_params.py': `# System Parameters Configuration\n\nMAX_LATENCY_MS = 100\nREDUNDANCY_LEVEL = 3\nHEARTBEAT_INTERVAL_SEC = 0.5\n\ndef validate_config():\n    return True`,
    'system/orchestrator/orchestrator.c': `/* System Orchestration Engine */\n#include "orchestrator.h"\n\nvoid sys_start_node(uint8_t node_id) {\n    /* Start system node sequence */\n}`,
    'system/orchestrator/Makefile': `CC=gcc\nall: orchestrator.o\norchestrator.o: orchestrator.c\n\t$(CC) -c orchestrator.c -o build/orchestrator.o`,
    'system/README.md': `# System Architecture\n\nHigh-level system orchestration and parameter definitions.`,
  });
  const [activeFile, setActiveFile] = useState('system/config/sys_params.py');
  const [terminalOutput, setTerminalOutput] = useState([
    '$ archtech-sys --v3-engine initialized',
    '[INFO] System Workspace ready. Select a tab to begin.',
  ]);

  const fileTree = (() => {
    const root = { name: 'ArchTech_System', type: 'dir', children: [] };
  ...
  useEffect(() => {
    if (isGenerating.syrs || isGenerating.arch) setTerminalOpen(true);
  }, [isGenerating]);

  const handleGenerateDoc = async (type) => {
    setIsGenerating(prev => ({ ...prev, syrs: type === 'SYRS', arch: type !== 'SYRS' }));
    if (type === 'SYRS') setSyrsDoc(''); else setArchDoc('');
    try {
  ...
  const handleCompile = () => {
    setIsCompiling(true);
    setTerminalOutput(prev => [...prev, `$ Running system validation for ${activeFile}...`]);
    setTimeout(() => {
      setTerminalOutput(prev => [...prev, 'System configuration validated successfully. 0 issues detected.']);
      setIsCompiling(false);
    }, 1400);
  };

  const handleExport = async () => {
    const content = activeTab === 'syrs' ? syrsDoc : activeTab === 'arch' ? archDoc : codeFiles[activeFile];
    if (exportFormat === 'odt') {
      try {
  ...
  const isGenActive = activeTab === 'syrs' ? isGenerating.syrs : isGenerating.arch;
  const currentDoc = activeTab === 'syrs' ? syrsDoc : archDoc;

  const TABS = [
    { id: 'syrs', label: 'SYRS Document', icon: <BookOpen size={13} /> },
    { id: 'arch', label: 'Architecture', icon: <LayoutTemplate size={13} /> },
    { id: 'code', label: 'System Config', icon: <Code2 size={13} /> },
  ];
      ...
      <div style={{
        ...
        {activeTab === 'code' && sidebarOpen && (
          <div style={{
            ...
            <SectionHeader
              title="Configuration"
              subtitle={`${Object.keys(codeFiles).length} params`}
              action={
          ...
          <div style={{
            ...
            <div style={{
              ...
              {TABS.map(t => (
                <TabPill
                  ...
                  badge={
                    t.id === 'syrs' && isGenerating.syrs ? 'live'
                    : t.id === 'arch' && isGenerating.arch ? 'live'
                    : null
                  }
                />
              ))}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button onClick={() => setTerminalOpen(o => !o)} style={{
                ...
                Console
                <StatusDot active={isGenerating.syrs || isGenerating.arch || isCompiling} />
              </button>
            </div>
          </div>
              ...
              <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  {activeTab === 'code' ? (
                    ...
                    <TiptapEditor
                      content={syrsDoc} onChange={setSyrsDoc}
                      project={activeProject} requirementId="SYRS_DOC"
                      className="h-full border-none shadow-none rounded-none"
                    />
                  ) : (
                    <TiptapEditor
                      content={archDoc} onChange={setArchDoc}
                      project={activeProject} requirementId="ARCH_DOC"
                      className="h-full border-none shadow-none rounded-none"
                    />
                  )}
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{
                      ...
                      background: isGenerating.syrs || isGenerating.arch ? C.accentLight : C.bg,
                      color: isGenerating.syrs || isGenerating.arch ? C.accent : C.textMuted,
                      display: 'flex',
                    }}>
                      {isGenerating.syrs || isGenerating.arch
                    ...
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.text }}>
                        {isGenerating.syrs || isGenerating.arch ? 'Generating…' : 'Console'}
                      </div>
